...
def import_subcatchments(data, nodes=None, find_widths=False, mask_array=None):
    ...
    try:
        ...
        print('determining subcatchments wth polygon shapefile')
    ...
    #For auto-calculating subwatersheds:
        ...
        grid = Grid.from_raster(data, data_name='dem')       #load DEM raster (if data_name='dem', returns an object called grid.dem)
        ...
        threshold = 50                                           #set snapping threshold - only cells with acc values higher than this will be considered (20-50 is good)
        grid.acc[grid.acc<=threshold] = 0                        #set all cells with value <= threshold equal to zero
        xyPour = grid.snap_to_mask(grid.acc, xy, return_dist=False)     #snap pour points to the high-accumulation cells closest to the desired nodes
        if find_widths==True:
        ...
        for i in range(len(xyPour)):       #loop over snapped points
            ...
            if (pt==points).all(1).any():  #check if current point is already in the list
                node = np.asarray([nodes.X.iloc[i], nodes.Y.iloc[i]])   #get XY coordinates of original unsnapped node
                if (node==points).all(1).any():                         #check if original node is already in the list
                    print('pour point', pt, 'already snapped to! original point', node, 'already in list! skipping this point') #if yes, skip this point
                    points.append([np.nan,np.nan])                      #assign nans to this point (to preserve indexing while skipping it)
                else: 
                    print('pour point', pt, 'already snapped to! replacing with original node:', [nodes.X.iloc[i], nodes.Y.iloc[i]] ) #print notification message
                    points.append([nodes.X.iloc[i], nodes.Y.iloc[i]])  #append original point value to the list
            else:
                points.append(pt)          #otherwise append snapped point to list
        ...
        for i in range(len(xyPour)):                    #loop over pour points
            ...
            if find_widths==True:
                dist = grid.flow_distance(x, y, data='dir', weights=cell_dist, dirmap=dirmap, out_name='dist', xytype='label', inplace=False) #calculate the flow distance from each cell in the subcatchment to the outlet of that subcatchment (in cells)
        ...
        dem,dema,xmin,xmax,xres,dx,ymin,ymax,yres,dy,zmin,zmax,zres = importDEM(data)   #get grid resolution info from DEM
        catchIDs, counts = np.unique(catchs[~np.isnan(catchs)], return_counts=True) #get list of unique non-NaN values in array, and number of times each value occurs
        catchIDs = np.array(catchIDs, dtype=int)                   #convert to int
        areas = counts*dx*dy                                  #calculate subcatchment areas: ncells * cell width * cell height
        if find_widths==True:
...
def get_timeseries(rainfallOld, baseflowOld, scaNew, scaOld, timestrings):
...
def set_raingages(catch, raintype='VOLUME', tintvl='1:00', snowctch=1.0, datatype='TIMESERIES'):
    ...
    *SWMMargs:  can be either a single value or a list of same length as number of junctions. See SWMM doc for details.'''
    ...
    catchIDs = np.array(catchIDs+1, dtype=int)      #convert to int      
    raingages = pd.DataFrame()                      #create empty dataframe to store info
    raingages['Name'] = [str(s) for s in catchIDs]  #assign raingage name to be same as subcatch name and convert to 1-based indexing
    raingages['RainType']   = raintype
    raingages['tIntvl']     = tintvl
    raingages['SnowCtch']   = snowctch
    raingages['DataSource'] = [datatype+' Rainfall'+str(s) for s in catchIDs] #assign a rainfall timeseries to each raingage of the same number
...
def set_inflows(nodes, catch, par='FLOW', partype='FLOW', unitfactor=1.0, scalefactor=1.0, baseval='', basepattern=''):
    ...
    *SWMMargs:  can be either a single value or a list of same length as number of junctions. See SWMM doc for details.
    ...
    inflows['TimeSeries']  = ['Baseflow'+str(s+1) for s in catchIDs]            #assign timeseries name from subcatch indices but with 1-based indexing
...
def set_subcatchments(nodes, catch, areas, width='auto', pctimpv=0.25, slope=0.25, curblen=0, snowpack='', plot=False):
    ...
    *SWMMargs:  can be either a single value or a list of same length as number of junctions. See SWMM doc for details.'''
    ...
    try:
        ...
        outlets['subcatchment'] = catchIDs+1                             #convert subcatch index to 1-based and store (doesn't matter for raster data, but this way is compatible with polygons)
...
def set_subareas(catch, nimperv=0.2, nperv=0.1, simperv=0.15, sperv=0.05, pctzero=100, routeto='IMPERVIOUS', pctrouted=100):
    ...
    *SWMMargs:  can be either a single value or a list of same length as number of junctions. See SWMM doc for details.'''
    ...
    subareas['Subcatchment'] = ['S'+str(s+1) for s in catchIDs] #assign subcatchments (add an S to name to distinguish from nodes)
...
#########################################################################################
def set_infiltration(catch, maxrate=3, minrate=0.5, decay=4, drytime=7, maxinfil=0):
...
def set_timeseries(datalist, datatypes, catch, timestrings):
    '''Format timeseries data so that it can be used in the SWMM input file. 
      Each timeseries must correspond to a subcatchment.
       ...
       catch:       array or raster with an integer in each cell indicating the 0-based ID of the subcatchment that cell belongs to (subcatchment ID number is the same as the ID number of the node that subcatchment drains to) (returnable by import_subcatchments())
       timestrings: list of strings indicating the datetime stamp of each value - all timeseries should have same datetime indices'''
    
    catchIDs  = np.unique(catch[~np.isnan(catch)])          #get list of unique non-NaN values in array, and number of times each value occurs
    ...
    for ind, data in enumerate(datalist):          #loop over types of data
        name = datatypes[ind]
        for sc in catchIDs:                        #loop over subcatchment indices
            toadd.Value = data[sc].values          #store calculated data in new dataframe
            toadd.Name = name + str(int(sc))       #assign name etc.
            toadd.datetime = timestrings
            toadd.type = name
            toadd.catch = sc                       #assign subcatch index 
    ...
    #timeseries = timeseries[timeseries.Value != 0]                                          #remove all lines where value is zero (SWMM only needs non-zero timesteps)
...
def plot_map(nodes, links, dim=3, poly=None, fig=None, ax=None, labels=False, c='k', a=1):
    ...
    if dim==2:
        ...
        ax.scatter(nodes[nodes.type=='outfall'].X, nodes[nodes.type=='outfall'].Y, c='r', s=30, alpha=a)    #plot spring nodes in a dif color
        for ind in links.index:                                                              #loop over link indices
            ax.plot((fromX.iloc[ind],toX.iloc[ind]),(fromY.iloc[ind],toY.iloc[ind]), c=c, alpha=a, linewidth=1)    #plot links
        if labels==True:
    ...
    if dim==3:
        ...
        ax.scatter(nodes[nodes.type=='outfall'].X, nodes[nodes.type=='outfall'].Y, nodes[nodes.type=='outfall'].Z,
                   c='r', s=20, alpha=a)                                                     #plot spring nodes in a dif color
        for ind in links.index:                                                              #loop over link indices
            ax.plot((fromX.iloc[ind],toX.iloc[ind]),(fromY.iloc[ind],toY.iloc[ind]),(fromZ.iloc[ind],toZ.iloc[ind]), c=c, alpha=a, linewidth=0.5)    #plot links
        if labels==True:
            for ind in nodes.index:                                                          #loop over node indices
...
def getQ(Q, swmm_output_file, columns):           
    '''Get spring discharge timeseries from SWMM binary output file using swmmtoolbox package.
       Inputs:
       Q: empty dataframe with index set to be the same datetimes as in Qobs timeseries
       swmm_output_file: binary SWMM output file path (ex: 'modelname.out')
       columns: list of column name strings to use (one per outfall node)
    ...
    for i, node in enumerate(nodenames):                                         #loop over available spring nodes
        ...
        Q = Q.rename(columns={nodeInflow.columns[0]: columns[i]})                #rename columns
    return Q
